...
import {editTask, insertTask} from "../../../services/TaskServices";
import {IoIosCloseCircle} from "react-icons/io";
import {FaCheck} from "react-icons/fa";

export function TaskEditing({isVisible, setIsVisible, taskToBeEdited, updateTask}) {

    const [title, setTitle] = useState(taskToBeEdited.title)
    const [description, setDescription] = useState(taskToBeEdited.description)
    const [token, setToken] = useState(taskToBeEdited.token)
    ...
    const handleSubmit = async (event) => {
        ...
        console.log("teste", taskToBeEdited)
        try{
            const e = await editTask(formattedTask)
